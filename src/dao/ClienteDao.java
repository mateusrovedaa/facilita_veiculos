    ...
    public Cliente consultarId(int id) {
        ...
        try {
            ...
            while (resultadoQ.next()) {
                ...
                cliente.setRg(resultadoQ.getString("rg"));
                cliente.setCpf(resultadoQ.getString("cpf"));
                cliente.setCidade(resultadoQ.getString("cidade"));
    ...
    public void popularTabela(JTable tabela, String criterio) {
        ...
        try {
            resultadoQ = ConexaoBD.getInstance().getConnection().createStatement().executeQuery(""
                    + "SELECT count(*) FROM clientes AS c WHERE c.NOME ILIKE '%" + criterio + "%' AND "
                    + "c.id IN (SELECT id FROM clientes WHERE NOME ILIKE '%" + criterio + "%' LIMIT 50)");
